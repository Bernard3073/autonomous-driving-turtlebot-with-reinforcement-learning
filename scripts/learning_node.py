...
# X_INIT = -2
# Y_INIT = 1
# THETA_INIT = 180 # [degrees]

X_INIT = -2
Y_INIT = 0
THETA_INIT = 0 # [degrees]
        ...
        while not rospy.is_shutdown():
            ...
            if step_time > MIN_TIME_BETWEEN_ACTIONS:
                ...
                if episode > MAX_EPISODES:
                    ...
                    text = '--------------------------------------- \r\n\r\n'
                    text = text + \
                        'MAX EPISODES REACHED(%d), LEARNING FINISHED ==> ' % MAX_EPISODES + \
                        dt_string_stop + '\r\n'
                    text = text + \
                        'Simulation time: %d:%d:%d  h/m/s \r\n' % (
                            sim_time_h, sim_time_m, sim_time_s)
                    text = text + \
                        'Real time: %d:%d:%d  h/m/s \r\n' % (
                    ...
                    np.savetxt(LOG_FILE_DIR+'/StateSpace.csv',
                               state_space, '%d')
                    np.savetxt(LOG_FILE_DIR+'/steps_per_episode.csv',
                               steps_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/reward_per_episode.csv',
                               reward_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/T_per_episode.csv',
                               T_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/EPSILON_per_episode.csv',
                               EPSILON_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/reward_min_per_episode.csv',
                               reward_min_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/reward_max_per_episode.csv',
                               reward_max_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/reward_avg_per_episode.csv',
                               reward_avg_per_episode, delimiter=' , ')
                    np.savetxt(LOG_FILE_DIR+'/t_per_episode.csv',
                               t_per_episode, delimiter=' , ')
                    ...
                    if crash or ep_steps >= MAX_STEPS_PER_EPISODE:
                        ...
                        text = '---------------------------------------\r\n'
                        if crash:
                            text = text + '\r\nEpisode %d ==> CRASH {%.2f,%.2f,%.2f}    ' % (
                                episode, x_crash, y_crash, theta_crash) + dt_string
                        elif ep_steps >= MAX_STEPS_PER_EPISODE:
                            text = text + '\r\nEpisode %d ==> MAX STEPS PER EPISODE REACHED {%d}    ' % (
                                episode, MAX_STEPS_PER_EPISODE) + dt_string
                        else:
                        ...
                        if not robot_in_pos:
                            ...
                            # init pos
                            if RANDOM_INIT_POS:
                                (x_init, y_init, theta_init) = robotSetRandomPos(
                                    setPosPub)
                            else:
                                (x_init, y_init, theta_init) = robotSetPos(
                                    setPosPub, X_INIT, Y_INIT, THETA_INIT)
                        ...
                        # First action
                            ...
                            (state_ind, x1, x2, x3, x4) = scanDiscretization(
                                state_space, lidar)
                            crash = checkCrash(lidar)
                            # make sure it's convergence
                            (action, status_strat) = epsiloGreedyExploration(
                        ...
                        # Rest of the algorithm
                            ...
                            (Q_table, status_uqt) = updateQTable(
                                Q_table, prev_state_ind, action, reward, state_ind, alpha, gama)

                            if EXPLORATION_FUNCTION == 1:
                                (action, status_strat) = softMaxSelection(
                                    Q_table, state_ind, actions, T)
                            else:
                                (action, status_strat) = epsiloGreedyExploration(
                                    Q_table, state_ind, actions, T)
